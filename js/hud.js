class HUD extends PIXI.Container {
  constructor() {
    ...
    PIXI.Loader.shared.add('helmet', '../mat/helmet.png');
    PIXI.Loader.shared.add('battery', '../mat/battery-full.png');
    PIXI.Loader.shared.add('oxygen', '../mat/diving.png');
  ...
  newBar(param = {}) {
    let { x, y, width = 200, height = 30, color = 0x3498db, alpha = .7, icon } = param;
    let HUD = this;
    ...
    return new (function () {
      ...
      this.init = function () {
        ...
        this.view.x = x < 0 ? HUD.app.screen.width + x : x;
        this.view.y = y < 0 ? HUD.app.screen.height + y : y;
        ...
        this.bar.back.filters = [new PIXI.filters.DropShadowFilter()];
      ...
      this.loadIcon = function (resources) {
        let pic = new PIXI.Sprite(resources[icon].texture);
        pic.y = -5;
        pic.width = 45;
        pic.height = 45;
  ...
  newMap(param = {}) {
    let { x, y, radius = 80, color = 0x2c3e50, alpha = .9 } = param;
    let HUD = this;
    ...
    return new (function () {
      ...
      this.init = function () {
        ...
        this.view.beginFill(0xecf0f1, .7);
  ...
  newPenal(param = {}) {
    let { color = 0x2c3e50, alpha = .9 } = param;
    let HUD = this;
    ...
    return new (function () {
      this.init = function () {
        ...
        this.unit = { w: HUD.app.screen.width / 16, h: HUD.app.screen.height / 16 };

        this.view.lineStyle(5, color, alpha);
        //#region 1
        this.view.beginFill(0xecf0f1, .5);
        ...
        //#endregion
        //#region 2
        this.view.beginFill(0xecf0f1, .9);
        this.view.moveTo(this.unit.w * 0, this.unit.h * 0);
        this.view.lineTo(this.unit.w * 6, this.unit.h * 0);
        this.view.lineTo(this.unit.w * 5, this.unit.h * .5);
        this.view.lineTo(this.unit.w * 2, this.unit.h * .5);
        this.view.lineTo(this.unit.w * 1, this.unit.h * 1);
        this.view.lineTo(this.unit.w * 0, this.unit.h * 0);
        this.view.endFill();
        //#endregion
        //#region 3
        this.view.beginFill(0xecf0f1, .8);
        ...
        //#region 4
        this.view.beginFill(0xecf0f1, .9);
        this.view.moveTo(this.unit.w * 6, this.unit.h * .3);
        ...
        this.view.lineTo(this.unit.w * 10, this.unit.h * .3);
        this.view.lineTo(this.unit.w * 6, this.unit.h * .3);
        this.view.endFill();
        //#endregion
        ...
        this.view.lineTo(this.unit.w * 11, this.unit.h * .5);
        this.view.lineTo(this.unit.w * 14, this.unit.h * .5);
        this.view.lineTo(this.unit.w * 15, this.unit.h * 1);
        this.view.lineTo(this.unit.w * 16, this.unit.h * 0);
        this.view.endFill();
        //#endregion
  ...
  newAlert(param = {}) {
    ...
    return new (function () {
      this.init = function () {
        ...
        this.view.x = HUD.app.screen.width - HUD.panel.unit.w * 4.15;
        this.view.y = 50;
  ...
  tester() {
    let helmetTester = setInterval(() => { if (--this.helmet.value < 1) { clearInterval(helmetTester); } }, Math.random() * 1000);
    let batteryTester = setInterval(() => { if (--this.battery.value < 1) { clearInterval(batteryTester); } }, Math.random() * 1000);
    let oxygenTester = setInterval(() => { if (--this.oxygen.value < 1) { clearInterval(oxygenTester); } }, Math.random() * 1000);

    let alertTester = setInterval(() => { this.alert.text = {str: 'Enemy Attack !!!', sec: 10, color: 0xe74c3c} }, 500);
  }
}